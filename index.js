...
//middle ware for providing the body
...
app.get("/jokes/:id",(req,res)=>{
  ...
  const foundJoke=jokes.find((joke)=>joke.id===Requested_id);
  res.json(foundJoke);
...
app.post("/jokes",(req,res)=>{
 ...
 jokes.push(newJoke);
 //accessing the last joke in the console enviroment 
...
const replacementJoke={
  ...
  // the incoming text is req.body.text
  jokeText: req.body.text,
  jokeType: req.body.type,
};

const search_index = jokes.findIndex((joke)=>joke.id===id);
jokes[search_index]=replacementJoke;
response.json(replacementJoke);
});
...
app.delete("/all",(req,res)=>{
  ...
  if(userKey===masterKey){
    ...
    res.sendStatus(200);
      ...
      res.status(404);
      res.json({error:"You are not an appropriate person for this"});
    }
 });
